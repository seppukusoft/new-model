...
// Functions to update map display
function applyColor(state, newColor) {
    const colorMapping = {
...
function changeDesc(state, num) {
    ...
    if (target) {
        target.description = ["ME1", "ME2", "NE2"].includes(state) ? 
        `${num} Electoral Vote` : 
        `${num} Electoral Votes`;
    }
}
...
function oddsDesc(state, hPercent, tPercent) {
    ...
    if (target && !isNaN(hPercent) && !isNaN(tPercent)) {
        target.description += `<br>Harris: ${Math.round(hPercent)} out of 100<br>Trump: ${Math.round(tPercent)} out of 100`;
    }
}
